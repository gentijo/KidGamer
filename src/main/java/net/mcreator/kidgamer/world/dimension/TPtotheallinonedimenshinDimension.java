...
package net.mcreator.kidgamer.world.dimension;
...
import net.mcreator.kidgamer.item.TPtotheallinonedimenshinItem;
import net.mcreator.kidgamer.block.EnderpearlblockBlock;
...
public class TPtotheallinonedimenshinDimension extends KidGamerModElements.ModElement {
	@ObjectHolder("kid_gamer:t_ptotheallinonedimenshin_portal")
	public static final CustomPortalBlock portal = null;
	public TPtotheallinonedimenshinDimension(KidGamerModElements instance) {
		super(instance, 222);
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				ObfuscationReflectionHelper.setPrivateValue(WorldCarver.class, WorldCarver.CAVE, new ImmutableSet.Builder<Block>()
						...
						.add(Blocks.BEACON).build(), "field_222718_j");
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("kid_gamer:t_ptotheallinonedimenshin"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("t_ptotheallinonedimenshin_portal", Vector3i::compareTo, 300);
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("t_ptotheallinonedimenshin_portal",
					Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
							.setRegistryName("t_ptotheallinonedimenshin_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new TPtotheallinonedimenshinItem().setRegistryName("t_ptotheallinonedimenshin"));
	}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("kid_gamer:t_ptotheallinonedimenshin"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("kid_gamer:t_ptotheallinonedimenshin")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == EnderpearlblockBlock.block;
		};
				...
				for (int l1 = -1; l1 < 2; ++l1) {
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0 ? EnderpearlblockBlock.block.getDefaultState() : Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, EnderpearlblockBlock.block.getDefaultState(), 3);
					}
				}
			}
